  ...
  (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (err instanceof Error && err.message.includes('not found')) {
        res
          .status(404)
          .json({ error: 'Order not found', message: err.message });
      } else {
        next(err);
      }
    }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (
        ...
        (err.message.includes('Customer') || err.message.includes('Product'))
      ) {
        res
          .status(400)
  ...
  (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!status) {
        res
          .status(400)
          .json({ error: 'Validation error', message: 'Status is required' });
        return;
      }
